#!/usr/bin/env python3
"""
Evaluate the Socratic model using local Gemma-4 as the Socratic Judge.
This script matches the evaluation logic of socratic-llm-repo.

Usage:
    python evaluate_with_gemma_judge.py --num-samples 10
"""
...
# Append socratic-llm-repo/src to path to import Scores, Example, Evaluation schema
...
def escape_template(str_template: str) -> str:
    """Escapes '{' and '}' characters for string formatting, preserving placeholder keys."""
...
def clean_tutor_response(answer: str) -> str:
    """Removes extra turns or student simulations from the generated response."""
    # Split by common role markers that are generated in error
...
def extract_and_parse_json(text: str) -> dict:
    """Extracts JSON string from response (handling markdown blocks) and parses it."""
    cleaned = text.strip()
...
def main():
    ...
    for idx, prompt in enumerate(tqdm(eval_prompts, desc="Evaluating")):
        ...
        try:
            tutor_resp = tutor_client.chat.completions.create(
        ...
        if not tutor_answer.startswith("[ERROR]"):
            try:
                ...
                error_msg = str(e)
                # Retry once without seed if there's a strict schema issue
                try:
                    # Fallback regex search to recover if response format had minor issue
                    parsed_json = extract_and_parse_json(raw_evaluation)
                    evaluation = Evaluation.model_validate(parsed_json)
                    error_msg = None
