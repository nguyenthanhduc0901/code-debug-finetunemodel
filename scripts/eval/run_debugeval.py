...
def post_process_error_code_localization_zero_shot(response):
    match = re.search('<Answer>(.*?)</Answer>', response, re.DOTALL)
    content = match.group(1).strip() if match else response.strip()
    opt_match = re.search('\\(([A-D])\\)', content)
    if opt_match:
        return f'({opt_match.group(1)})'
    opt_match = re.search('\\b([A-D])\\b', content)
    if opt_match:
        return f'({opt_match.group(1)})'
...
def post_process_code_repair_zero_shot(response):
    for label in ['Correct_Solution:', 'Corrected_Solution:', 'Corrected Solution:', 'CorrectSolution:', '_Correct_Solution:', '_Correct_Solution{', '_Correct_:', '_FIX_:', '_FIX_', 'Answer_Code:', 'Lowered_Code:', 'Answer:']:
        if label in response:
            ...
            inner_blocks = re.findall('```(?:code|cpp|java|python|Python)?\\s*(.*?)\\s*```', code_part, re.DOTALL)
            if inner_blocks:
                return inner_blocks[0].strip()
            return code_part
    code_blocks = re.findall('```(?:code|cpp|java|python|Python)?\\s*(.*?)\\s*```', response, re.DOTALL)
    valid_blocks = [b.strip() for b in code_blocks if b.strip() and 'correct_solution' not in b.lower() and (b.strip() != 'and')]
    if valid_blocks:
...
def post_process_code_review_zero_shot(response):
    patterns = ['buggy code snippet is \\*\\*?(Code-[AB])\\*\\*?', 'incorrect code snippet is \\*\\*?(Code-[AB])\\*\\*?', 'buggy snippet is \\*\\*?(Code-[AB])\\*\\*?', '\\*\\*?(Code-[AB])\\*\\*? is the buggy snippet', '\\*\\*?(Code-[AB])\\*\\*? is the incorrect snippet', 'buggy code is \\*\\*?(Code-[AB])\\*\\*?', 'incorrect code is \\*\\*?(Code-[AB])\\*\\*?', 'buggy is \\*\\*?(Code-[AB])\\*\\*?', 'incorrect is \\*\\*?(Code-[AB])\\*\\*?', 'bug is in \\*\\*?(Code-[AB])\\*\\*?', '(Code-[AB]) is the buggy']
    ...
    def single_query(idx_prompt):
        ...
        try:
            ...
            if model_type in ['qwen', 'qwen-sft']:
                ...
                chat_prompt = prompt
            response = client.completions.create(model=model_name, prompt=chat_prompt, max_tokens=safe_max_tokens, temperature=0.2, top_p=0.95)
...
def judge_single_problem(idx_item_raw_prompt):
    ...
    if os.path.exists(input_path) and os.path.exists(answer_path):
        ...
        res_first, _, _, _ = judgeLib.judge(file_dir=source_file, input_dir=os.path.join(input_path, '1.in'), answer_dir=os.path.join(answer_path, '1.out'), timeLimit=10, memoryLimit=1024, test_id=0, cleanup=False)
        if res_first == 'CE':
            passed_all = False
            ...
            if test_cnt > 1:
                ...
                def run_single_test(t_idx):
                    res, _, _, _ = judgeLib.judge(file_dir=source_file, input_dir=os.path.join(input_path, f'{t_idx + 1}.in'), answer_dir=os.path.join(answer_path, f'{t_idx + 1}.out'), timeLimit=10, memoryLimit=1024, test_id=t_idx, cleanup=False)
                    return res
...
def run_task4_review(client, model_type, model_name, data, limit, verbose):
    ...
    print(f'  Evaluating responses...')
    for i, (item, raw_output_normal, raw_output_reverse, prompt_normal, prompt_reverse) in enumerate(zip(filtered_data, raw_outputs_normal, raw_outputs_reverse, prompts_normal, prompts_reverse)):
...
def main():
    ...
    for task_num, res in results.items():
        task_name = {1: 'BUG Localization', 2: 'BUG Identification', 3: 'Code Repair', 4: 'Code Review'}[task_num]
