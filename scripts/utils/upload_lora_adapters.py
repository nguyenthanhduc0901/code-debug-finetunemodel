...
def patch_readme(readme_path, local_base_path, base_model_id):
    ...
    # Replace absolute local path with proper HF Model ID
    patched_content = content.replace(local_base_path, base_model_id)
    # Also handle slightly different paths if LLaMA-Factory generated them
...
def main():
    ...
    token = args.token
    if not token:
        # Require HF_TOKEN environment variable or parameter instead of hardcoding
    ...
    for item in MODELS_TO_UPLOAD:
        ...
        # Patch README first to avoid Hugging Face Hub metadata errors
        readme_path = os.path.join(local_dir, "README.md")
        patch_readme(readme_path, local_base_path, base_model_id)
        
        try:
            # Create repository on Hugging Face (Private by default)
