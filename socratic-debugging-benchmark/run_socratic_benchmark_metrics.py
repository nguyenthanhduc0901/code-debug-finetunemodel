...
def main(use_chat_prompt, generation_mode, num_responses, dataset_path, eval_method, api_url=None, model_name=None):
    path = os.path.dirname(os.path.abspath(__file__))
    ...
    if api_url:
        ...
        output_dir = "results/local_model"
        ...
        for i, turn in enumerate(dialogue):
            ...
            if use_chat_prompt:
                if generation_mode == "multiple":
                    ...
                    if api_url:
                        response = target_model.generate_turn(
                ...
                if i == 0 and generation_mode == "cot":
                    ...
                    if api_url:
                        introspection = target_model.generate(introspecton_prompt)
                    else:
                ...
                if generation_mode == "cot":
                    ...
                    if api_url:
                        ...
                        response = target_model.generate(model_prompt)
            ...
            if generation_mode == "multiple" or generation_mode == "cot":
                if api_url:
                    ...
                    if "<code>" in response:
                        remove_code = extract_tag_content(response, "code")
    ...
    if api_url:
        if eval_method != "thoroughness":
            scores = metrics_computer.compute(
                responses, dataset_references, contexts=prompts
            )
        else:
            scores = metrics_computer.compute_thoroughness(
                responses, dataset_references, contexts=prompts
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Local model API endpoint base URL (e.g. http://localhost:8001/v1)",
    )
    parser.add_argument(
        "--model_name",
        type=str,
